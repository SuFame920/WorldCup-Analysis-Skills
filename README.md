...
[简介](#-简介) · [实战战报](#-实战战报校准闭环真的奏效) · [为什么](#-为什么需要它) · [核心特性](#-核心特性) · [工作原理](#️-工作原理) · [目录结构](#-目录结构) · [安装](#-安装) · [使用](#-使用方法) · [输出示例](#-输出示例) · [设计原则](#-设计原则) · [记忆系统](#️-记忆与自反思) · [信源](#-数据信源) · [FAQ](#-faq) · [免责声明](#️-免责声明)
...
> ### 比分命中率 **0/4 → 3/4**,只隔了一个反思周期。

这不是"挑好看的赛果"展示,而是这个 Skill 最独特的卖点——**反思 → 修正 → 下次预测**的闭环**真的在加信号**。

- **首批(2026-06-21,G/H 组 4 场):** 我系统性押"小球 + 受让方 + 强队小净胜球",结果**比分 0/4、大小球 1/4**。赛后反思归因到"把单一叙事套到每场",蒸馏出一条弱先验:**别给强队净胜球封顶太低**,写回 `calibration-summary.md`。
- **次批(2026-06-22,I/J 组 4 场):** 仅把这条修正作为 `Channel H` 弱先验载入,**比分跳到 3/4、胜平负 4/4**,法国一场**五项全中**。

### 2026-06-22 四场:赛前预测 vs 真实结果

| 比赛 | 赛前预测(核心) | 真实结果 | 命中 |
|---|---|---|---|
| 🇫🇷 法国 vs 伊拉克 🇮🇶 | 法国胜 / 让 2 过盘 / **比分 3-0** / 大小 3 球 / 半全胜胜 | **3-0**(HT 1-0) | ✅✅✅✅✅ **5/5** |
| 🇦🇷 阿根廷 vs 奥地利 🇦🇹 | 阿根廷胜 / 让 1 过盘 / **比分 2-0** / 半全胜胜 | **2-0**(HT 1-0) | ✅✅✅✅ 4/5(大小✗) |
| 🇳🇴 挪威 vs 塞内加尔 🇸🇳 | 挪威不败 / 偏大球 / 受让方有值 | **3-2**(HT 1-0) | ✅胜平负 ✅大小(让球走盘) |
| 🇩🇿 阿尔及利亚 vs 约旦 🇯🇴 | 阿尔及利亚胜 / **比分 2-1**(次选) / 受让方有值 | **2-1**(HT 0-1) | ✅胜平负 ✅比分(让球走盘) |

**逐市场汇总:** 胜平负 **4/4** · 比分 **3/4** · 让球 2/4(+2 走盘,方向均对) · 大小球 2/4 · 半全场 2/4。

### 诚实的反面:残留偏差也照样落盘

闭环不只记成功。次批暴露出**两个更精细的残留偏差**,已写成新的弱先验:① 大小球错向是"场景分型用反"(开放局押了小、绞杀局押了大),不是系统性偏差;② 新发现"半场平"默认偏差(有早段终结点的一方上半场就领先)。**别为纠一个错而矫枉过正**也被写进流程。

> 🔍 **可核验、无事后诸葛:** 所有赛前预测在开赛前就写入 `memory/records/`(带时间戳),赛后只填真实结果与打分,绝不回改原判;打分用 Brier / log-loss。完整复盘见 [`memory/calibration-summary.md`](memory/calibration-summary.md)。
